      ...
      <main className="auth-page-blobs flex flex-1 items-center justify-center overflow-hidden px-6 py-8 relative">
        ...
        >
          ...
          <div>
            ...
            <h1 className="text-[1.75rem] font-bold tracking-[-0.02em] m-0">Create account</h1>
          </div>
          ...
          <label className="text-text-muted text-[0.85rem] font-semibold grid gap-[0.4rem]">
            ...
            <div className={inputIconWrap}>
              ...
              <input name="password" type="password" minLength="6" value={form.password}
                onChange={handleChange} placeholder="Min. 6 characters" required
                autoComplete="new-password" className={inputWithIcon} />
            </div>
          </label>
          ...
          >
            {loading ? (
              <><span className="spinner spinner-white" />Creating account…</>
            ) : "Create account"}
          </button>

          <p className="text-text-muted text-center mt-2">
            Already have an account? <Link to="/login" className="text-primary font-semibold hover:text-primary-hover">Log in</Link>
          </p>
        </form>
      </main>
    </div>
